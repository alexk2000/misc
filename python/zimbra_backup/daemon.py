...
class Daemon(object):
    ...
    def get_config_item(self, option):
    ...
    def __get_start_lock(self):
        ...
        for sock in psutil.net_connections(kind="unix"):
            if sock.laddr == '@{appName}'.format(appName=self.__config['appName']):
        ...
        self.__applock.bind('\0{appName}'.format(appName=self.__config['appName']))
